...
function _storeArtifacts(projectRoot, operationName, fingerprint, operation, outputs) {
  ...
  try {
    ...
    const chosenPath = _artifactCandidatePath(root, chosen);
    const manifest = _artifactManifestRead(chosenPath);
    if (manifest === null ||
        !_artifactStoreVerified(chosenPath, operationName, fingerprint, manifest.outputs)) {
      throw new Error('chosen artifact candidate is not valid');
    }

    return _artifactSelect(root, chosen);
  } catch (error) {
    if (staging !== null) {
