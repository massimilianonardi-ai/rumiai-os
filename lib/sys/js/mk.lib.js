...
function _normalizeOperation(value, context, version) {
  ...
  const allowed = version === 1
    ...
    : new Set(['prerequisites', 'requirements', 'action', 'when', 'outputs', 'failure']);
...
function _deriveProviderOperation(providerName, provider, item) {
  ...
  return {
    ...
    definition: {prerequisites: provider.prerequisites.slice(), requirements: provider.requirements.slice(), action: substituteAction, when: null, outputs: {}, failure: 'stop'},
    ...
    if (ready === undefined) {
      ...
      for (const provider of plan.providers) {
        if (provider.state !== 'completed' && provider.unsatisfiedRequirements.length > 0 &&
            model.providers[provider.name].prerequisites.every(ref => _satisfied(runtime, ref))) {
