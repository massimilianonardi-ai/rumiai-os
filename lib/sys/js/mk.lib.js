...
function _normalizeOperation(value, context, version) {
  ...
  const allowed = version === 1
    ...
    : new Set(['prerequisites', 'action', 'when', 'outputs', 'failure']);
...
function _normalizeProviderMap(value, context) {
  ...
  for (const name of Object.keys(value)) {
    ...
    _assertAllowedKeys(provider, new Set(['type', 'collection', 'prerequisites', 'action']), `${context}: provider ${name}`);
...
function _normalizeProfiles(value, version) {
  ...
  for (const name of Object.keys(value)) {
    ...
    const allowed = version === 1
      ...
      : new Set(['environment', 'goals', 'operations', 'collections', 'providers']);
...
function _loadProjectConfig(configPath) {
  ...
  const allowed = version === 1
    ...
    : new Set(['version', 'environment', 'goals', 'operations', 'profiles', 'collections', 'providers']);
...
function _terminal(runtime, name) {
  ...
  return result !== undefined || runtime.skipped.has(name);
}
...
function _satisfied(runtime, name) {
  ...
  const result = runtime.results[name];
  return runtime.skipped.has(name) || (result !== undefined && result.ok);
}
...
function _deriveProviderOperation(providerName, provider, item) {
  ...
  return {
    ...
    definition: {prerequisites: provider.prerequisites.slice(), action: substituteAction, when: null, outputs: {}, failure: 'stop'},
...
function _resolveV2(projectRoot, model, requestedGoals, runtime) {
  ...
  function visitConditionDependencies(condition) {
    const refs = new Set(); _conditionOperationReferences(condition, refs);
  ...
  for (const name of Array.from(reachableOperations).sort(_byteCompare)) {
    const operation = operations[name];
    const observes = new Set(); _conditionOperationReferences(operation.when, observes);
    const condition = _evaluateCondition(projectRoot, model, runtime, operation.when);
    let state;
  ...
  return {version: 2, goals: requestedGoals.slice(), collections, providers, operations: operationEntries, _operations: operations};
}
...
function _executeV2(projectRoot, model, requestedGoals) {
  const runtime = {results: {}, skipped: new Set(), collectionState: {}, providerMembers: {}, providerState: {}};
  const baseEnvironment = _callerEnvironment(); _applyEnvironment(baseEnvironment, model.environment);
  while (true) {
    ...
    if (ready === undefined) _error('execution cannot make progress; unresolved conditional/provider dependencies remain');
...
function _publicPlan(plan) {
  const copy = Object.assign({}, plan); delete copy._operations; return copy;
}
  ...
  try {
    ...
    if (request.showGoal !== null) {
      ...
      if (model.version === 1) {
        ...
        const runtime = {results: {}, skipped: new Set(), collectionState: {}, providerMembers: {}, providerState: {}};
        const plan = _publicPlan(_resolveV2(project.root, model, [request.showGoal], runtime));
    ...
    if (request.plan) {
      const runtime = {results: {}, skipped: new Set(), collectionState: {}, providerMembers: {}, providerState: {}};
      const plan = _publicPlan(_resolveV2(project.root, model, request.goals, runtime));
