...
FUNCTIONS
    ...
    ipc_destroy channel
        ...
        Removes a channel that was created successfully but has not begun an
        ipc_open rendezvous.

        ipc_destroy MUST NOT run concurrently with ipc_open and MUST be used
        only before either endpoint has begun ipc_open. Removing FIFO pathnames
        while an endpoint is blocked in open can strand the peer.
        ...
        If a valid channel pathname no longer exists, ipc_destroy succeeds.
